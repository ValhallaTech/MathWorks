...
The production build is generated by **SvelteKit** via **`@sveltejs/adapter-netlify`**,
which writes the prerendered static assets to the `build` directory (read from
`build.publish` in `netlify.toml`).
...
These mirror the CI pipeline (`.github/workflows/ci.yml`), which runs lint,
check, test:coverage, and build.
